...
var metaMap = {
	...
	'tabs': 'CustomTab',
	// 'sharingRules': 'SharingRules', // TODO: not supported without reading file contents (get each fillname and specify in xml)
...
var subMeta = ['fields', 'validationRules', 'webLinks'];

exports.subMetaContains = function(metadata)
{
	return subMeta.includes(metadata);
};

exports.packageWriter = function(metadata, apiVersion) {

	apiVersion = apiVersion + '.0' || '37.0';
	...
	for (var type in metadata) {
		...
		if (metadata.hasOwnProperty(type)) {
			...
			{
				console.log('** Error - Unknown meta: ' + type);
				continue;
			}
...
exports.buildPackageDir = function (dirName, name, metadata, packgeXML, destructive, destructiveOnly, cb) {
